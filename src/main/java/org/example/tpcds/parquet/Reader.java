...
public class Reader {
  ...
  /**
   * Read a list of parquet files but only the given fields.
   * 
   * @param files    File names of parquet input
   * @param keyField Key Field name
   * @param namesArr Array of field names to be read.
   * @return An boject with the file records.
   * @throws IOException Error reading file.
   */
  public static Parquet getParquetData(List<String> files, String keyField, String... namesArr)
      throws IOException {
    Map<Integer, Group> simpleGroups = new HashMap<>();
    List<Type> fields = null;
    List<String> names = Arrays.asList(namesArr);
   ...
   * @throws IOException Error reading file.
   */
  public static void runPerRow(List<String> files, Consumer<Group> consumer, String... arrNames)
      throws IOException {
    List<String> names = Arrays.asList(arrNames);
    // for (String filePath : files) {
    files.stream().parallel().forEach(filePath -> {
